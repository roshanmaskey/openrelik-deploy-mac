...
if ! command -v podman &> /dev/null; then
  echo -e "\033[1;31m ⚠️  Error: podman is not installed. Please install podman before running this script.\033[0m"
  echo -e "     Follow \033[1mhttps://podman.io/docs/installation\033[0m to install podman for your platform.\n"
  echo -e "     Install podman using Homebrew: brew install podman\n"
  exit 1
fi

# Check if Docker Compose is installed
if ! command -v podman-compose &> /dev/null; then
  echo -e "\033[1;31m ⚠️  Error: podman compose is not installed. Please install podman-compose before running this script.\033[0m"
  echo -e "     Follow \033[1m\033[0mhttps://podman.io/docs/installation to install podman compose for your platform.\n"
  echo -e "     Install podman-compose using Homebrew: brew install podman-compose\n"
  exit 1
fi
...
#BASE_DEPLOY_URL="https://raw.githubusercontent.com/roshanmaskey/openrelik-deploy-mac/main/docker"
...
podman-compose up -d  --quiet --remove-orphans
...
podman-compose exec openrelik-server bash -c "(cd /app/openrelik/datastores/sql && alembic upgrade head)"
...
podman-compose exec openrelik-server \
